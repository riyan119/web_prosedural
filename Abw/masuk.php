...
<div id="utama">
	...
	<div id="inputan">
		<form  method="POST" action="" >
			<div>
				<input type="text" name="user" placeholder="Masukkan Username" class="ig"/>
			</div>
			<div style="margin-top:10px">
				<input type="password" name="pass" placeholder="Masukkan Password" class="ig"/>
			</div>
			<div style="margin-top :10px;	">
		...
		if  ($login)  {
			...
			if ($cek >=1) {
				if ($data['level']=="admin") {
					@$_SESSION['admin']= $data['kd_login'];
					header("location: index.php");
				} else if ($data['level']=="operator") {
					@$_SESSION['operator'] = $data['kd_login'];
					header("location: index.php");
				}
				} else {
					echo "login gagal";
